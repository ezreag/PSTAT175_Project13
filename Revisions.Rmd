...
plot(gender.KM, fun = "cloglog", col = colorlines, xlab = "Months",
...
industry.KM <- survfit(stagSurv ~ ind, data = turnover)
plot(industry.KM, fun = "cloglog", col = colorlines, xlab = "Months",
     ylab = "Log(-Log) survival", main = "Industry Log(-Log) curves")
legend(x="topleft", title="Legend", legend=c("G1", "G2", "G3"), fill=c("blue","pink","red"))
```
...
profession.KM <- survfit(stagSurv ~ prof, data = turnover)
plot(profession.KM, fun = "cloglog", col = colorlines, xlab = "Months",
...
plot(traffic.KM, fun = "cloglog", col = colorlines, xlab = "Months",
...
plot(greywage.KM, fun = "cloglog", col = colorlines, xlab = "Months",
...
plot(way.KM, fun = "cloglog", col = colorlines, xlab = "Months",
...
```{r, include=FALSE}
### log-log plots of continuous covariates `selfcontrol` and `anxiety`
par(mfrow=c(1,2))
# Profession Log(-Log) Plot
profession.KM <- survfit(stagSurv ~ as.factor(anxiety), data = turnover)
plot(profession.KM, fun = "cloglog", col = colorlines, xlab = "Months",
     ylab = "Log(-Log) survival", main = "Anxiety Log(-Log) curves")
# legend(x="topleft", title="Legend", legend=c("G1", "G2", "G3"), fill=c("blue","pink","red"))
# Traffic Log(-Log) Plot
traffic.KM <- survfit(stagSurv ~ as.factor(selfcontrol), data = turnover)
plot(traffic.KM, fun = "cloglog", col = colorlines, xlab = "Months",
     ylab = "Log(-Log) survival", main = "SelfControl Log(-Log) curves")
```
...
```{r}
cox.zph(CPH.Groupfit)
cox.zph(CPH.all)
```

The graphs are mostly parallel
...
# 4.0 Conclusions
```{r, include=FALSE}
summary(coxph(formula = stagSurv ~ age + ind + prof + traffic + greywage + way + 
...
```{r}
# report confidence intervals as hazard rates??? 
# confidence interval
summary(CPH.Groupfit)$conf.int[,-c(2)]
```
...
Hazard represents the risk of quitting. `trafficempjs` increases the hazard rate by 108%. `profG2` increases 80%. `age` increases 2%. `indG3` (1-0.46=0.54) decreases hazard rate by 54%. `grewwagewhite` decreases 40% (Pham H, 2018).  
  
Using these findings, we can conclude that the traffic factor, `empjs` significantly increases the chances of quitting as well as belonging to profession group 2 (Commercial, Engineering, Marketing, PR).  

```{r, include=FALSE}
# Plotting particular hazard rates with respect to the baseline Hazard. 
# non-essential, no need to include

# new references
# https://stats.stackexchange.com/questions/25317/how-to-calculate-predicted-hazard-rates-from-a-cox-ph-model
# https://stats.stackexchange.com/questions/83892/understanding-coxph-output-in-r

# summary(survfit(CPH.Groupfit))
baseline <- basehaz(CPH.Groupfit)
plot(baseline$time, baseline$hazard, type='l',main="Hazard rates", xlab="Months", ylab="hazard rate") 
# numbers based on exp(-coef) for age
lines(baseline$time, 0.5554792*baseline$hazard, col="coral") # profG2
lines(baseline$time, 0.4791*baseline$hazard, col="blue") # trafficempjs
lines(baseline$time, 1.6795587*baseline$hazard, col="grey") # greywagewhite
lines(baseline$time, 2.1804211*baseline$hazard, col="darkgreen") # indG3
legend(x="topleft", title="Legend", legend=c("baseline", "profG2", "trafficempjs", "greywagewhite","indG3"), fill=c("black","coral","blue","grey","darkgreen"))
```
